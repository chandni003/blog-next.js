    ...
    <main className="min-h-screen bg-background text-foreground transition-colors duration-300 pb-32">
      ...
      <article className="container mx-auto px-6 lg:px-10 py-16 flex flex-col lg:flex-row gap-16 max-w-7xl">
        ...
        <aside className="hidden lg:flex flex-col gap-8 w-16 h-fit sticky top-32">
          <Button variant="outline" size="icon" className="rounded-2xl size-14 border-border bg-card/30 hover:bg-primary/5 hover:text-primary hover:border-primary/30 transition-all shadow-sm">
          ...
          <Button variant="outline" size="icon" className="rounded-2xl size-14 border-border bg-card/30 hover:bg-primary/5 hover:text-primary hover:border-primary/30 transition-all text-muted-foreground/40 shadow-sm">
        ...
        <div className="flex-1 max-w-3xl lg:px-10">
          ...
          {post?.thumbnailUrl && (
            <div className="w-full aspect-video rounded-[2.5rem] bg-muted overflow-hidden border border-border shadow-2xl mb-16 group relative">
              <img
          ...
          <div className="prose prose-lg dark:prose-invert prose-indigo max-w-none 
              ...
              prose-img:rounded-[2rem] prose-img:border prose-img:border-border prose-img:shadow-2xl">
            <div
              className="space-y-6"
          ...
          {/* Author Sign-off */}
          <div className="mt-20 pt-10 border-t border-border flex items-center gap-6 group">
            <div className="size-16 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground font-bold text-2xl transition-transform group-hover:scale-105">
              {post?.authorId?.[0]?.toUpperCase() || "N"}
            </div>
            <div className="space-y-0.5">
              <h4 className="font-bold text-xl">{post?.authorId || "Nexus Content Lab"}</h4>
              <p className="text-sm text-muted-foreground font-medium opacity-80">Full-Stack Architect specializing in high-performance cloud environments.</p>
            </div>
          </div>
        </div>

        {/* Right Sidebar - Meta info / Navigation */}
        <aside className="hidden xl:flex flex-col gap-10 w-80 h-fit sticky top-32">
          <div className="p-8 rounded-[2rem] bg-card/40 backdrop-blur-md border border-border shadow-xl relative overflow-hidden group">
            <div className="absolute top-0 right-0 size-20 bg-primary/5 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity" />
            <h5 className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary mb-6">Article Overview</h5>
            <ul className="space-y-6 text-sm font-bold text-muted-foreground">
              <li className="flex items-center gap-3 text-foreground transition-colors">
                <div className="size-1.5 rounded-full bg-primary shadow-sm shadow-primary/50" /> Introduction
              </li>
              <li className="flex items-center gap-3 opacity-60 hover:opacity-100 cursor-pointer transition-all border-l border-border pl-3 ml-0.5 hover:border-primary hover:text-primary">Architectural Review</li>
              <li className="flex items-center gap-3 opacity-60 hover:opacity-100 cursor-pointer transition-all border-l border-border pl-3 ml-0.5 hover:border-primary hover:text-primary">Pattern Analysis</li>
              <li className="flex items-center gap-3 opacity-60 hover:opacity-100 cursor-pointer transition-all border-l border-border pl-3 ml-0.5 hover:border-primary hover:text-primary">Final Benchmarks</li>
            </ul>
          </div>

          <div className="p-10 rounded-[2.5rem] bg-primary/5 border border-primary/20 text-center relative overflow-hidden group">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--primary)_0%,_transparent_100%)] opacity-[0.05] group-hover:opacity-[0.1] transition-opacity" />
            <h5 className="text-xl font-bold mb-3 relative z-10 text-foreground">Technical Brief</h5>
            <p className="text-xs text-muted-foreground mb-8 relative z-10 leading-relaxed">Join 5k experts receiving deep-architectural breakdowns weekly.</p>
            <Button className="w-full rounded-2xl py-6 font-bold bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:scale-105 transition-all relative z-10">
              Secure Subscription
            </Button>
          </div>
        </aside>
